...
import * as dataFormat from 'pages/CRUD/Operation_cd/table/Operation_cdDataFormatters';

import actions from 'actions/operation_cd/operation_cdListActions';
...
const Operation_cdTable = () => {
  ...
  const count = useSelector((store) => store.operation_cd.list.count);
  const modalOpen = useSelector((store) => store.operation_cd.list.modalOpen);
  const rows = useSelector((store) => store.operation_cd.list.rows);
  const idToDelete = useSelector((store) => store.operation_cd.list.idToDelete);
  ...
  const getOperation_cdCSV = async () => {
    const response = await axios({url: '/operation_cd?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'operation_cdCSV.csv'
    link.click()
  };
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="operation_cd" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>{humanize('Operation_cd')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/operation_cd/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Operation_cd'
...
export default Operation_cdTable;
