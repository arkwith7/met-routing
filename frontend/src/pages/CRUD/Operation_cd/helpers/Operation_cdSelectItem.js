...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/operation_cd/autocomplete`, { params });
  return response.data;
}

const Operation_cdSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.operation_cd.hasPermissionToCreate
});

export default connect(select)(
  Operation_cdSelectItem,
);
