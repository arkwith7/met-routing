...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/doc_extraction_items/autocomplete`, { params });
  return response.data;
}

const Doc_extraction_itemsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.doc_extraction_items.hasPermissionToCreate
});

export default connect(select)(
  Doc_extraction_itemsSelectItem,
);
