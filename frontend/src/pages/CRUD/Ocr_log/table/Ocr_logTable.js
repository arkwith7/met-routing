...
import * as dataFormat from 'pages/CRUD/Ocr_log/table/Ocr_logDataFormatters';

import actions from 'actions/ocr_log/ocr_logListActions';
...
const Ocr_logTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Registration No', title: 'registration_no'},{label: 'Doc Name', title: 'doc_name'},{label: 'Is Classification', title: 'is_classification'},
  ...
  const count = useSelector((store) => store.ocr_log.list.count);
  const modalOpen = useSelector((store) => store.ocr_log.list.modalOpen);
  const rows = useSelector((store) => store.ocr_log.list.rows);
  const idToDelete = useSelector((store) => store.ocr_log.list.idToDelete);
  ...
  const getOcr_logCSV = async () => {
    const response = await axios({url: '/ocr_log?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'ocr_logCSV.csv'
    link.click()
  };
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="ocr_log" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>{humanize('Ocr_log')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/ocr_log/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Ocr_log'
