...
import * as dataFormat from 'pages/CRUD/Doc_master/table/Doc_masterDataFormatters';

import actions from 'actions/doc_master/doc_masterListActions';
...
const Doc_masterTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Doc Name', title: 'doc_name'},{label: 'Doc Class Cd', title: 'doc_class_cd'},{label: 'Doc Subclass Cd', title: 'doc_subclass_cd'},{label: 'Doc Class Name', title: 'doc_class_name'},{label: 'Doc Subclass Name', title: 'doc_subclass_name'},{label: 'Doc Name Alias', title: 'doc_name_alias'},{label: 'Doc Keyword', title: 'doc_keyword'},
  ...
  const count = useSelector((store) => store.doc_master.list.count);
  const modalOpen = useSelector((store) => store.doc_master.list.modalOpen);
  const rows = useSelector((store) => store.doc_master.list.rows);
  const idToDelete = useSelector((store) => store.doc_master.list.idToDelete);
  ...
  const getDoc_masterCSV = async () => {
    const response = await axios({url: '/doc_master?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'doc_masterCSV.csv'
    link.click()
  };
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="doc_master" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>{humanize('OCR 서류정보')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/doc_master/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Doc_master'
                : 'http://localhost:8080/api-docs/#/Doc_master'
            }
          >
            API documentation for doc_master
          </LinkMaterial>
        </div>
      </Widget>
...
export default Doc_masterTable;
