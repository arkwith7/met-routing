...
import * as dataFormat from 'pages/CRUD/Insurance_cd/table/Insurance_cdDataFormatters';

import actions from 'actions/insurance_cd/insurance_cdListActions';
...
const Insurance_cdTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Code', title: 'code'},{label: 'Code Name', title: 'code_name'},{label: 'Is Operation', title: 'is_operation'},
  ...
  const count = useSelector((store) => store.insurance_cd.list.count);
  const modalOpen = useSelector((store) => store.insurance_cd.list.modalOpen);
  const rows = useSelector((store) => store.insurance_cd.list.rows);
  const idToDelete = useSelector((store) => store.insurance_cd.list.idToDelete);
  ...
  const getInsurance_cdCSV = async () => {
    const response = await axios({url: '/insurance_cd?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'insurance_cdCSV.csv'
    link.click()
  };
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="insurance_cd" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>{humanize('수가코드정보')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/insurance_cd/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Insurance_cd'
...
export default Insurance_cdTable;
