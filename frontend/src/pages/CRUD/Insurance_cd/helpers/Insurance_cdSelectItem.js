...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/insurance_cd/autocomplete`, { params });
  return response.data;
}

const Insurance_cdSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.insurance_cd.hasPermissionToCreate
});

export default connect(select)(
  Insurance_cdSelectItem,
);
