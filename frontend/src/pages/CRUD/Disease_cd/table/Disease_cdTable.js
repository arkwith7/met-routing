...
import * as dataFormat from 'pages/CRUD/Disease_cd/table/Disease_cdDataFormatters';

import actions from 'actions/disease_cd/disease_cdListActions';
...
const Disease_cdTable = () => {
  ...
  const count = useSelector((store) => store.disease_cd.list.count);
  const modalOpen = useSelector((store) => store.disease_cd.list.modalOpen);
  const rows = useSelector((store) => store.disease_cd.list.rows);
  const idToDelete = useSelector((store) => store.disease_cd.list.idToDelete);
  ...
  const getDisease_cdCSV = async () => {
    const response = await axios({url: '/disease_cd?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'disease_cdCSV.csv'
    link.click()
  };
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="disease_cd" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>{humanize('질병코드정보')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/disease_cd/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Disease_cd'
...
export default Disease_cdTable;
