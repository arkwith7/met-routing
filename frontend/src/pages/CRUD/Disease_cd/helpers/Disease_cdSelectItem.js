...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/disease_cd/autocomplete`, { params });
  return response.data;
}

const Disease_cdSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.disease_cd.hasPermissionToCreate
});

export default connect(select)(
  Disease_cdSelectItem,
);
